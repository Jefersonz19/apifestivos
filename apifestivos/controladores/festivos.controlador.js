...
exports.listarFestivos = async (req, res) => {
  try {
    ...
    const bd = await conexionBd();

    // Obtener los festivos de la base de datos
    const tiposFestivos = await bd.collection('tipos').find({}).toArray();
    ...
    for (const tipo of tiposFestivos) {
      switch (tipo.id) {
        ...
        case 3: // Festivos de Semana Santa
          for (const festivo of tipo.festivos) {
            const fechaPascua = fechas.obtenerDomingoPascua(parseInt(year));
    ...
    for (const tipo of tiposFestivos) {
      switch (tipo.id) {
        ...
        case 3: // Código para calcular los festivos de la tabla id=3
          const tiposFestivosSemanaSanta = await bd.collection('tipos').findOne({ id: 3 });
        ...
        case 4: // Código para calcular los festivos de la tabla id=4
          const tipoFestivoId4 = await bd.collection('tipos').findOne({ id: 4 });
          if (!tipoFestivoId4) {
...
exports.obtenerInicioSemanaSanta = async (req, res) => {
  try {
    ...
    const inicioSemanaSanta = fechas.obtenerDomingoPascua(parseInt(year));
